...
func (r *ConnectorController) GetPoliciesDecisions(c *gin.Context) {
	// Parse request
	var request policy.GetPolicyDecisionsRequest
	bbb, _ := ioutil.ReadAll(c.Request.Body)
	if err := json.Unmarshal(bbb, &request); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	// Get asset metadata from catalog connector
	requestToCatalog := &catalog.GetAssetRequest{
	...
	var response policy.GetPolicyDecisionsResponse
