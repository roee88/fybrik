...
type DataCatalogDummy struct {
	dataDetails map[string]catalog.GetAssetResponse
}

func (d *DataCatalogDummy) GetAssetInfo(in *catalog.GetAssetRequest, creds string) (*catalog.GetAssetResponse, error) {
...
func NewTestCatalog() *DataCatalogDummy {
	dummyCatalog := DataCatalogDummy{
		dataDetails: make(map[string]catalog.GetAssetResponse),
	}
	...
	// TODO(roee88): no need to marshal/unmarshal for creating a connection
	...
	dummyCatalog.dataDetails["s3-external"] = catalog.GetAssetResponse{
	...
	dummyCatalog.dataDetails["s3"] = catalog.GetAssetResponse{
		ResourceMetadata: catalog.ResourceMetadata{
			Name:      "xxx",
			Geography: &geo,
			Tags:      tags,
	...
	dummyCatalog.dataDetails["s3-csv"] = catalog.GetAssetResponse{
		ResourceMetadata: catalog.ResourceMetadata{
			Name:      "xxx",
			Geography: &geo,
			Tags:      tags,
	...
	dummyCatalog.dataDetails["db2"] = catalog.GetAssetResponse{
		ResourceMetadata: catalog.ResourceMetadata{
			Name:      "xxx",
			Geography: &geo,
			Tags:      tags,
	...
	dummyCatalog.dataDetails["kafka"] = catalog.GetAssetResponse{
		ResourceMetadata: catalog.ResourceMetadata{
			Name:      "xxx",
			Geography: &geo,
			Tags:      tags,
